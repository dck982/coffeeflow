...
void send_block_start(uint16_t number, uint16_t crc) {
  common::FlashCtrlPayload p{};
  p.subcmd = common::FlashSubCmd::kBlockStart;
  p.block_number = number;
  p.block_crc16 = crc;
  const common::Frame frame = p.pack();
  can_link::send_message(common::MessageType::kFlashCtrl, common::Dest::kSensors, frame.data(), 5);
}
...
bool begin_upload(uint32_t size) { if(g.uploading||g.sending||size==0) return false; g.partition=esp_partition_find_first(ESP_PARTITION_TYPE_DATA,static_cast<esp_partition_subtype_t>(0x40),"ota_staging"); if(!g.partition||size>g.partition->size-kHeaderBytes||esp_partition_erase_range(g.partition,0,g.partition->size)!=ESP_OK) return false; g.size=size;g.written=g.sent=0;g.crc=common::Crc32Incremental{};g.uploading=true;return true; }
