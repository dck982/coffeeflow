...
def boitier_ac(
    ...
    hauteur_vis: hauteur des murs vis / muret / dalle, depuis le lit
    ...
    puit_depuis_ouest: distance du centre du puits à la face ouest extérieure
    puit_depuis_nord: distance du centre du puits à la face nord extérieure
    ...
    if hauteur_vis < vis_z:
        reject(
            f"hauteur_vis {hauteur_vis} is under the screw ({vis_z} mm): raise it",
            param="hauteur_vis",
        )
    ...
    puit_cx = aile_x + puit_depuis_ouest
    puit_cy = y_max - puit_depuis_nord
    if puit_depuis_ouest < wall + pad_r:
        reject(
            f"puit_depuis_ouest {puit_depuis_ouest} puts the well pad into "
            f"the west wall: raise it above {wall + pad_r:.1f}",
            param="puit_depuis_ouest",
        )
    ...
    if puit_cx + pad_r > muret_x:
        reject(
            f"puit_depuis_ouest {puit_depuis_ouest} runs the well into the "
    ...
    # Slab between the E/W walls, top flush with the 15 mm wall tops.
    ...
    # Same bar as the vis legs: 15 mm tall, 18 mm in Y, 20 mm from west face.
    body = body + _bb(
        muret_x, plat_y0, 0.0, muret_x + wall, y_max, hauteur_vis
    )

    # North face is open only at the housing interior. From z = 15 the
    # north wall runs the full X width (lintel sitting on the slab).
    body = body + _bb(
