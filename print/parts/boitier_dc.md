...
Size: 50.00 x 95.00 x 27.00 mm, 20618.6 mm3, 1 solid, 124 faces
Checks: clean
<!-- /AUTO -->
...
- Fond imprimé sur le lit. Contour via `dc_contour()` sur `bb_overall()` / `bb_top()` (`system.bbox`) : mur est à x=`boitier_int_aile_x` (50), nord à y=`boitier_int_y` (95), chanfrein SW de (0, `boitier_int_chanfrein`) à (`chanfrein`, 0) — cote mesurée 15, plus de slider. Marche à x=`boitier_int_marche_x` (10,5), y=`boitier_int_marche_y` (49). `bb_bottom()` prend toute la largeur (`aile_x`), plus seulement la marche — `magnet_spacer` s'en sert. Les compartiments Wago / CAN Pal prennent une bbox conteneur, pas des X/Y bruts. `couvercle_acdc` importe `dc_contour`.
...
- XIAO (SE) : deux inserts M2 (`system.INSERT_M2`) au nord de la zone, deux plots pleins Ø3 plus au sud, un mur fin (1,26mm) à l'ouest. Plus de puits d'aimant sous le module : les aimants sud sont dans `magnet_spacer`.
...
- Ne pas remettre un puits sous le XIAO ni au centre du Wago SW : les aimants sud sont dans `magnet_spacer`.
- Ne pas recoder les trous des plots spacer : `magnet_spacer_pins(wall)`, mêmes centres que les plots. `bb_bottom` est large comme `aile_x`, pas comme la marche.
