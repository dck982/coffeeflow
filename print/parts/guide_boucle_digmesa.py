...
def guide_boucle_digmesa(hauteur_passage=40.0, jeu_tube=2.5, distance_trou=25.0, draft=False):
    ...
    if distance_trou < 13.0:
        reject("Distance trou au moins 13mm",param="distance_trou")        
    ...
    eye+=block(7,3,13,9,0,hauteur_passage-5)
    ...
    base-=Pos(trou_cx,0,-1)*extrude(hole,amount=4)
    body=eye+base
    if not draft:
